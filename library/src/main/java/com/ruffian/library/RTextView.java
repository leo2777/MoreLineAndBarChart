 ...
 * @author ZhongDaFeng
 */
public class RTextView extends androidx.appcompat.widget.AppCompatTextView {
    ...
    private void initAttributeSet(Context context, AttributeSet attrs) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            if (normalId != -1)
                mIconNormal = ContextCompat.getDrawable(context, normalId);
            if (pressedId != -1)
                mIconPressed = ContextCompat.getDrawable(context, pressedId);
            if (unableId != -1)
                mIconUnable = ContextCompat.getDrawable(context, unableId);
        }
